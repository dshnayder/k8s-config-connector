...
func TestSelectingCRDsByAPIGroups(t *testing.T) {
	// Define test CRDs for different API groups
	testCRD1 := `
apiVersion: apiextensions.k8s.io/v1
kind: CustomResourceDefinition
metadata:
  name: resources1.test1.cnrm.cloud.google.com
  labels:
...
kind: CustomResourceDefinition
metadata:
  name: resources2.test1.cnrm.cloud.google.com
  labels:
    cnrm.cloud.google.com/system: "true"
    cnrm.cloud.google.com/managed-by-kcc: "true"
spec:
...
kind: CustomResourceDefinition
metadata:
  name: resources3.test2.cnrm.cloud.google.com
  labels:
    cnrm.cloud.google.com/system: "true"
    cnrm.cloud.google.com/managed-by-kcc: "true"
spec:
...
kind: CustomResourceDefinition
metadata:
  name: resources4.test3.cnrm.cloud.google.com
  labels:
    cnrm.cloud.google.com/system: "true"
    cnrm.cloud.google.com/managed-by-kcc: "true"
spec:
...
`
	...
	// Create test manifest collections
	allCRDs := []string{testCRD1, testCRD2, testCRD3, testCRD4, testcontroller.SystemNs}
	test1GroupCRDs := []string{testCRD1, testCRD2, testcontroller.SystemNs}
	test1And2GroupCRDs := []string{testCRD1, testCRD2, testCRD3, testcontroller.SystemNs}

	tests := []struct {
		name              string
		manifests         []string
		apiGroupCRs       []customizev1alpha1.APIGroupInstallation
		expectedManifests []string
		hasError          bool
	}{
		{
			name:              "no APIGroupInstallation CRs, all CRDs are selected",
			manifests:         allCRDs,
			apiGroupCRs:       []customizev1alpha1.APIGroupInstallation{},
			expectedManifests: allCRDs,
		},
		{
			name:      "one APIGroupInstallation CR, only CRDs from that API group are selected",
			manifests: allCRDs,
			apiGroupCRs: []customizev1alpha1.APIGroupInstallation{
				{
					ObjectMeta: metav1.ObjectMeta{
			...
			expectedManifests: test1GroupCRDs,
		},
		{
			name:      "multiple APIGroupInstallation CRs, CRDs from those API groups are selected",
			manifests: allCRDs,
			apiGroupCRs: []customizev1alpha1.APIGroupInstallation{
				{
					ObjectMeta: metav1.ObjectMeta{
			...
			expectedManifests: test1And2GroupCRDs,
		},
		{
			name:      "non-KCC CRD in manifests causes error",
			manifests: append(testcontroller.GetManifestsWithNonKCCCRD(), testCRD1),
			apiGroupCRs: []customizev1alpha1.APIGroupInstallation{
				{
					ObjectMeta: metav1.ObjectMeta{
						Name: "test1.cnrm.cloud.google.com",
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, cr := range tc.apiGroupCRs {
				if err := c.Create(ctx, &cr); err != nil {
					t.Fatalf("error creating APIGroupInstallation %s: %v", cr.Name, err)
				}
			}
			...
			err := r.selectCRDsByAPIGroups(ctx, manifests)
			if tc.hasError {
				if err == nil {
					t.Fatalf("got nil, want an error")
				}
				...
				t.Fatalf("error selecting CRDs by API groups: %v", err)
			}
